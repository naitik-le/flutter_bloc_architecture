...
void main() {
  test('Offline SpaceX Launches API Benchmark', () {
    print('Starting offline SpaceX Launches API Benchmark...');
    ...
    final mockLaunchesList = List.generate(250, (index) {
      return {
        ...
        'details': 'Engine failure at 33 seconds and loss of vehicle. ' *
            (index % 3 + 1), // variable length details
        ...
        'links': {
          ...
          'article':
              'https://www.space.com/2196-spacex-falcon-1-launch-fails.html',
        },
        'failures': [
          if (index % 10 == 0) ...[
    ...
    print('Simulated Payload Size: ${payloadSizeKb.toStringAsFixed(2)} KB');
    ...
    print('JSON Decoding Time: ${jsonDecodeTimeMs}ms');

    // 3. Measure Model Mapping / Deserialization Time
    print('Mapping to models...');
    stopwatch.reset();
    stopwatch.start();
    final launches = decodedData
        .map((e) => LaunchModel.fromJson(e as Map<String, dynamic>))
        .toList();
    stopwatch.stop();
    final modelMappingTimeMs = stopwatch.elapsedMilliseconds;
    print('Model Mapping/Deserialization Time: ${modelMappingTimeMs}ms');
    print('Total Launches parsed: ${launches.length}');
  });
}
